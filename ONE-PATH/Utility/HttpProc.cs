...
namespace ONE_PATH.Utility
{
    public static class HttpProc
    {
        ...
        public class WebClient
        {
            ...
            /// 是否使用json格式协议标头
            /// </summary>
            private bool IsUseJsonHeader = false;


            ///<summary>  
            ///初始化WebClient类  
            ///</summary>  
            public WebClient(bool isuseJsonHeader = false)
            {
                IsUseJsonHeader = isuseJsonHeader;
            ...
            public string UploadFile(string URL, string textField, string fileField)
            {
                ...
                byte[] bufferheader = encoding.GetBytes(fileField);

                //添加文件数据  
                FileStream fs = new FileStream(filePath, FileMode.Open, FileAccess.Read);

                fileField = "\r\n";
                byte[] bufferend = encoding.GetBytes(fileField);


                fileField = "--" + BOUNDARY + "--\r\n";
                byte[] bufferend1 = encoding.GetBytes(fileField);
                //写入结束标记  
                requestHeaders.Add("Content-Length", (fs.Length + bufferheader.Length + bufferend.Length + bufferend1.Length).ToString());
                ...
                if (Proxy != null)
                {
                    clientSocket = Proxy.CreateConnection(URI.Host, URI.Port);
                }
                else
                {
                    clientSocket = new TcpClient(URI.Host, URI.Port);
                }
                ...
                //每次发送的字节数
                byte[] buffer4 = new byte[SEND_BUFFER_SIZE];
                count = 0;
                Stream sm = clientSocket.GetStream();

                sm.Write(bufferheader, 0, bufferheader.Length);
                UploadEventArgs e = new UploadEventArgs();
                e.totalBytes = fs.Length;
                ...
                do
                {
                    ...
                    //读取要发送的数据  
                    count = fs.Read(buffer4, 0, buffer4.Length);
                    //发送到服务器  
                    sm.Write(buffer4, 0, count);
                    //是否显示进度  
                    if (showProgress)
                    {
                        //触发事件  
                ...
                if (!isCanceled)
                {
                    sm.Write(bufferend, 0, bufferend.Length);
                    sm.Write(bufferend1, 0, bufferend1.Length);
                }
            ...
            public string UploadFileByStream(string url, string fileName, Stream stream)
            {
                int count = 0;
                string fileField = "--" + BOUNDARY + "\r\n";
                fileField += "Content-Disposition: form-data; name=\"file1\"; filename=\"" + fileName + "\"\r\n";
                fileField += "Content-Type: application/octet-stream\r\n\r\n";
                ...
                byte[] bufferend1 = encoding.GetBytes(fileField);
                //写入结束标记  
                requestHeaders.Add("Content-Length", (stream.Length + bufferheader.Length + bufferend.Length + bufferend1.Length).ToString());
                requestHeaders.Add("Content-Type", "multipart/form-data; boundary=" + BOUNDARY);
                requestHeaders.Add("Connection", "Keep-Alive");
                ...
                do
                {
                    ...
                    //读取要发送的数据  
                    count = stream.Read(buffer4, 0, buffer4.Length);
                    //发送到服务器  
                    sm.Write(buffer4, 0, count);
            ...
            private void WriteFileField(string fileField)
            {
                ...
                foreach (string var in strArr)
                {
                    ...
                    //添加文件数据  
                    FileStream fs = new FileStream(filePath, FileMode.Open, FileAccess.Read);
                    int byteslength = 50000;
                    buffer = new byte[byteslength];
                    postStream.Capacity += (int)fs.Length;
            ...
            {
                StreamFileName = "";
                requestHeaders.Add("Cache-Control", "no-store, must-revalidate");
                requestHeaders.Add("expires", "Wed, 26 Feb 1997 08:21:57 GMT");
                requestHeaders.Add("expires", "no-0");
            ...
            private void SendRequestData(string URL, string method, bool showProgress)
            {
                ...
                {
                    //System.IO.File.AppendAllText("c:\\aa.txt", "host:" + URI.Host + "    port:" + URI.Port);
                    //clientSocket = new TcpClient(URI.Host, URI.Port);
                    clientSocket = new TcpClient(URI.Host, URI.Port);
            ...
            ///</summary>  
            ///<returns>服务器响应文本</returns>  
            private string GetHtml(bool isUpload = false)
            {
                if (!isCanceled)
                {
                    MemoryStream ms = new MemoryStream();
                    SaveNetworkStream(ms, true, isUpload);//将网络流保存到内存流  
                    if (isUpload)
                    {
                        ms.Close();
                        return uploadResult;
            ...
            private void SaveNetworkStream(Stream toStream, bool showProgress, bool isUpload = false, bool IsReGetResult = false)
            {
                //获取要保存的网络流  
                ...
                if (responseHeaders.GetValues("Content-Disposition") != null)
                {
                    ...
                    if (header.Contains("="))
                    {
                        StreamFileName = header.Split('=')[1];
                    }
                }

                toStream.Position = 0;
                if (isUpload)//如果为上传文件 则需要判断是否获取到了上传结果
                {
                    StreamReader sr = new StreamReader(toStream);
                    uploadResult = sr.ReadToEnd();
                    sr.Close();
                    JsonObj jsonResult = JsonObj.Parse(uploadResult);
                    if (jsonResult.GetList().Count == 0 && !isReadAll)//说明获取上传文件结果未获取到
                    {
                        //间隔两秒再次获取
                        Thread.Sleep(50);

                        toStream = new MemoryStream();
                        SaveNetworkStream(toStream, showProgress, isUpload, true);
                    }
                }
                //关闭网络流和网络连接  
                NetStream.Close();
